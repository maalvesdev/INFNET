...
#No seu bloco de código Python no Deepnote, utilize o caractere `#` para escrever um comentário explicativo sobre a importância da programação.

#A programação é importante porque permite criar tecnologias que facilitam tarefas do dia a dia e resolvem problemas reais. Ela desenvolve o raciocínio lógico e o pensamento crítico. 
#Além disso, está presente em diversas áreas, ampliando oportunidades profissionais e impulsionando a inovação tecnológica.


#==Exercício 2 - Processo de Escrita e Hello World==
#Escreva um programa no Deepnote que utilize a função `print()` para exibir a mensagem: "Olá Mundo! Bem-vindo ao Cinema Digital".
print("Olá Mundo! Bem-vindo ao Cinema Digital")


#==Exercício 3 - Abordagem Programática e Atribuição==
#Declare uma variável chamada `disciplina` com o valor `""Python""` e uma variável chamada `turma` com o valor `101`. Utilize dois comandos `print()` separados para exibir o conteúdo de cada variável no terminal.
disciplina = "Python"
turma = 101
print(disciplina)
print(turma)
...
#No Deepnote, atribua os seguintes valores: `quantidade_macas` recebe `150` e `preco_maca` recebe `5.99`. Utilize a função `print()` para exibir primeiro a quantidade e depois o preço, em linhas separadas.
...
#Crie as variáveis `contratos = 12` e `faturamento = 2500.50`. Use a função `print()` para exibir o tipo da variável `contratos` e o tipo da variável `faturamento` utilizando a função `type()`.
contratos = 12
faturamento = 2500.50
...
#Crie uma variável chamada `sensor_ativo` e atribua a ela o valor booleano `True`. Em seguida, exiba o valor da variável no terminal utilizando `print()`. Altere seu valor para `False` e imprima novamente.
sensor_ativo = True
print(sensor_ativo)
sensor_ativo = False
print(sensor_ativo)
...
#Defina `venda_smartphones = 5400.00` e `venda_acessorios = 1250.50`. Crie uma variável `receita_total` que realize a soma e exiba o resultado final.
...
#Crie as variáveis `saldo_atual = 10000` e `valor_fatura = 3500`. Calcule o saldo restante e armazene em `saldo_final`. Exiba o valor de `saldo_final`.
saldo_atual = 10000
valor_fatura = 3500
...
#Com base na capacidade de 550kg e no peso unitário de 65kg, desenvolva um código que calcule o número máximo de sacas que podem ser embarcadas em um caminhão sem exceder o limite bem como a porcentagem de ocupação do peso do caminhão.
...
#Defina os valores para custo base (2500), alíquota (0.05) e suporte (150). Calcule, em apenas uma linha utilizando parênteses, o valor final que o cliente deverá pagar, garantindo que a ordem das operações respeite a regra de negócio descrita. 
#Exiba o total final.
custo_base = 2500
aliquota = 0.05
suporte = 150
...
#Crie um sistema que receba o registro de 345 minutos e realize a transformação para a grandeza de horas decimais. O resultado deve refletir a fração exata do tempo de uso para que o custo por hora seja aplicado posteriormente. 
